            ...
            {reviews?.map((review, index) => (
              <li key={index} className="bg-gray-100 p-2 rounded shadow-md">
                <div className="text-gray-800 textarea mb-2 bg-white">
                  ...
                  <div className='flex'>
                    <span > <img className='h-12 rounded-full' src={review.image ? review.image : avatar} alt={comment.userId} /></span>
                    <span className='uppercase font-bold flex items-center mx-3'>{review.userId}</span>
                    <span className='uppercase font-bold flex items-center mx-3'>{review.createdAt ? review.createdAt.split('T')[0] : '123123'}</span>
                  </div>
